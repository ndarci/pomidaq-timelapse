...
PYBIND11_MODULE(miniscope, m)
{
    ...
    py::class_<Miniscope>(m, "Miniscope")
        ...
        .def_property_readonly(
            ...
            &Miniscope::availableDeviceTypes,
            "Get a list of all Miniscope variants we can communicate with")
        .def(
            "load_device_config",
            &Miniscope::loadDeviceConfig,
            "Load harware definition for a given Miniscope device type")

        .def_property_readonly(
            "device_type", &Miniscope::deviceType, "get the name of the currently loaded Miniscope device type")
        ...
        .def_property_readonly(
            "is_running", &Miniscope::isRunning, "Is True if we are acquiring images from the Miniscope")
        .def_property_readonly("is_recording", &Miniscope::isRecording, "Is True if we are recording data")

        .def_property_readonly(
            "current_disp_frame",
            &Miniscope::currentDisplayFrame,
        ...
        .def_property(
            "video_filename", &Miniscope::videoFilename, &Miniscope::setVideoFilename, "The name of the saved video")
        .def_property("video_codec", &Miniscope::videoCodec, &Miniscope::setVideoCodec, "The video codec to use")
        .def_property(
            "video_container", &Miniscope::videoContainer, &Miniscope::setVideoContainer, "The video container to use")
        .def_property(
            "record_lossless",
            &Miniscope::recordLossless,
            &Miniscope::setRecordLossless,
            "Toggle lossless recording, if the codec supports it")

        .def_property(
            "min_fluor_display",
            &Miniscope::minFluorDisplay,
            &Miniscope::setMinFluorDisplay,
            "Minimum fluorescence to display")
        .def_property(
            "max_fluor_display",
            &Miniscope::maxFluorDisplay,
            &Miniscope::setMaxFluorDisplay,
            "Maximum fluorescence to display")
        .def_property_readonly(
            "min_fluor", &Miniscope::minFluor, "Minimum fluorescence (pixel value) in the current image")
        .def_property_readonly(
            "max_fluor", &Miniscope::maxFluor, "Maximum fluorescence (pixel value) in the current image")

        .def_property(
            "display_mode",
            &Miniscope::displayMode,
            &Miniscope::setDisplayMode,
